...
class Handler extends EventEmitter {
    ...
    async processCommand(interaction) {
		...
		if (!command.canRun(interaction)) {
			interaction.reply((await i18n(interaction)).t("You're not allowed to run this command."));
			return;
		}

		// Read arguments
		const args = command.runArgs(interaction);
		let actualArgs = {};
		if (args) {
			if (!await matchArguments(interaction, actualArgs, args)) {
				interaction.reply((await i18n(interaction)).t("Some of the provided arguments are invalid."));
				return;
			}
		}
		
		// Run through middleware
		const opts = command.runOpts(interaction);
		let actualOpts = {};
		if (!await this.#mw.run(interaction, actualOpts, opts, actualArgs)) return;

		if (opts.translatorRequired) {
		...
		if (opts.dbRequired && !opts.noReleaseDb) {
