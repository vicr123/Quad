...
class Dialog {
    ...
    constructor(user, channel, t, opts) {
        ...
        if (!opts.emoji) opts.emoji = "🚫"; // :no_entry_sign:
    ...
    reactionMade(message, emoji, userID) {
        if (message.id === this.#message.id && userID === this.#user.id && emoji.name === this.#opts.emoji) {
            this.cancel();
        }
    }
        ...
        return new Promise(async (res, rej) => {
            ...
            this.#message = await this.#channel.createMessage({
                embed: {
                    ...
                    footer: {
                        text: t("Mistake? React with {{emoji}} within 5 seconds.")
                    },
                    fields: this.#opts.fields
                }
            });
            ...
            this.#timeout = setTimeout(this.cont, 5000);
        })
    }
}
